...
echo "<br>".__("this plugin Powered By attributions Did U accept this ?","subt")." : ".__("yes","subt")." <input type='radio' name='subt_accept' ".$radio1." value='1'> - ".__("no","subt")."<input type='radio' name='subt_accept' ".$radio0." value='0'> (".__("If you dont accept this , plugin cant work Correctly","subt").")<br>\n";
...
$s = curl_init(); 
curl_setopt($s,CURLOPT_URL,"http://subscene.com/subtitles/".strtolower(str_replace(" ","-",$name))."/".strtolower(get_option("subt_lang"))); 
...
echo '<div class="subtitle_finder">';
...
$exit.='<br><a href="http://parsgraph.net/subtitle_finder" target="_blank">Powered By Subtitle Finder V 1</a>';
return $exit;
}
function subt_shortcode( $atts ) {
    return subtitle_finder($atts['movie']);
}
...
$_POST['movie']=str_replace("+","-",$_POST['movie']);
echo subtitle_finder($_POST['movie']);
}
}
?> </div></div> <?php
}
...
function subt_init() {
 load_plugin_textdomain( 'subt', false, 'subtitle_finder/langs' );
}
function header_function_subt(){
	echo "<link rel='stylesheet' href='".plugins_url()."/subtitle_finder/style.css' type='text/css' media='all' />\n";
}
